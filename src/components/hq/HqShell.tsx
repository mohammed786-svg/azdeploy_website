...
const NAV = [
  ...
  { href: "/hq/it-invoices", label: "IT client invoices", match: (p: string) => p.startsWith("/hq/it-") },
  { href: "/hq/receipts", label: "Receipts", match: (p: string) => p.startsWith("/hq/receipts") && !p.startsWith("/hq/it-") },
...
export default function HqShell({ children }: { children: React.ReactNode }) {
  ...
  async function logout() {
    ...
    if (typeof window !== "undefined") {
      ...
      window.localStorage.removeItem(HQ_API_LOCAL_STORAGE_KEY);
